...
5. 排序查询
    ...
    - 示例:
        ...
        books = models.Book.objects.order_by("-price")  ##降序排序
        for book in books:
...
6. 根据条件查询多条记录
    ...
    for book in books:
         ...
         books = models.Book.objects.filter(price=20,pub="清华大学出版社")
...
1. `__exact` : 等值匹配
    ...
    # select * from bookstore_author where id = 1;
    ```
...
2. `__contains` : 包含指定值
    ...
    # select * from bookstore_author where name like '%王%';
    ```
    
3. `__startswith` : 以 XXX 开始

    ```python
    Author.objects.filter(name__startwith='赵')
    # 等同于 select * from author where name like '赵%'
    ```
...
写一个网页，来修改图书的零售价。界面参照添加图书add_book.html

```python
路由：/bookstore/modbo/5
视图函数：def modbo_view(request,id)
模板:boostore/modbo.html
```
...
1. 删除单个对象
    ...
    - 示例:
        ...
        ```shell
        In [3]: books = models.Book.objects.get(id=15)
        In [4]: books = models.Book.objects.all()
        In [5]: books = models.Book.objects.get(pub="北京大学出版社")
        In [6]: books.delete()
        ```
...
- 使用MyModel.objects.raw()进行 数据库查询操作查询
    - 在django中，可以使用模型管理器的raw方法来执行select语句进行数据查询。只能执行查询操作。
    1. 语法: 
