...
```mysql
    ...
    3、改 ： （更新表记录）update 表名 set 字段1=值1,字段2=值2,... where 条件;
...
```mysql
    1、增 ： alter table 表名 add 字段名 数据类型;
...
```mysql
char(4)  varchar(4) （查找效率几乎一致，update时有差别） text blob  userid{}
解释：char(4) xxxx（不到4个用空格补）
     varchar(4) xxx+1 <255 
     varchar(400) 400+2 磁盘存储长度=400+2
```
...
eg2 : 查询AID1905和AID1903班的学生姓名及成绩
  select name,score from students where class in ('AID1905','AID1903');
```
...
分页：每页显示10条，显示第6页的内容
    limit(6-1)*10 10 (m-1)*n,n m为要显示的页数，n为每页显示数据条数
```
...
B树的特点->1.全部节点均包含索引(id)+数据('Tom_0')
          2.范围查询->从根节点遍历至指定数据

B+树的特点->1.非叶子节点只保存索引 [树宽度优于B树，从而降低了磁盘IO]
           2.叶子节点保存所有的索引和数据
           3.叶子节点之间相互连接，形成链表结构 
...
例:create table students_test(id int primary key auto_increment,name varchar(32),phone varchar(11),index(name),unique(phone));
```
