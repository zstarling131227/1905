...
| 如何解决csrf问题                                             | 采用token                                                    |
| Single Page web Application （SPA）是否会影响Search Engine Optimization（SEO）效果 | 会，前后端分离后，往往页面不存在静态文字【例如新闻的详细内容】 |
...
如何解决http无状态？
不分离时可以使用：
session:存储在服务器,使用时需要使用sessionid才能使用。(类似以门票)
cookie+session：存储在浏览器
分离式：采用token.（存在没有浏览器的情况）
...
SEO(搜索引擎优化)：根据搜索网站爬虫的user-agent返回纯静态【带文字内容】的网页。
...
>>> import base64
>>> base64.b64encode(b'zxl')
b'enhs'
>>> s=base64.b64encode(b'zxl')
>>> base64.b64decode(s)
b'zxl'
...
```linux
>>> payload = {'username': 'xixi', 'iss':'wo'}
>>> a = jwt.encode(payload, '123456', algorithm='HS256')
>>> a
b'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJ1c2VybmFtZSI6InhpeGkiLCJpc3MiOiJ3byJ9.J3bLm6ivdCHHAxVG1Iv_wuR2bBQAc1UDpdY7S6HvQ2c'
>>> b=jwt.decode(a,'123456',algorithms='HS256')##正常解码
>>> b
{'username': 'xixi', 'iss': 'wo'}
>>> b=jwt.decode(a,'123456',algorithms='HS256',issuer='wo')###校验正确
>>> b
{'username': 'xixi', 'iss': 'wo'}
>>> b=jwt.decode(a,'123456',algorithms='HS256',issuer='ss')##报错
```
...
```
#安装wheel文件 
tarena@tarena:~$ pip3 install /home/tarena/1905/month03/project03/projectb/PyJWT-1.4.0-py2.py3-none-any.whl
tarena@tarena:~$ pip3 freeze|gerp 'JWT'

```

```
安装压缩包：
tarena@tarena:~$ cd /home/tarena/1905/month03/project03/projectb
tarena@tarena:~/1905/month03/project03/projectb$ tar -zxvf django-cors-headers-3.0.2.tar.gz 
tarena@tarena:~/1905/month03/project03/projectb$ cd django-cors-headers-3.0.2/
tarena@tarena:~/1905/month03/project03/projectb/django-cors-headers-3.0.2$ ls
tarena@tarena:~/1905/month03/project03/projectb/django-cors-headers-3.0.2$ sudo python3 setup.py install
##过滤，查看版本。
tarena@tarena:~/1905/month03/project03/projectb/django-cors-headers-3.0.2$ pip3 freeze|grep 'cors'
输出django-cors-headers==3.0.2 表示安装成功。
tarena@tarena:~/1905/month03/project03/projectb/django-cors-headers-3.0.2$ pip3 freeze|grep 'Django'
输出结果必须为Django==1.11.8的版本。
```
...
```python
		...
		3，CORS_ORIGIN_ALLOW_ALL  布尔值  如果为True 白名单不启用（测试时设置为true,上线时设置为false）
		4，CORS_ORIGIN_WHITELIST =[
			"https://example.com"
		]###白名单（当CORS_ORIGIN_ALLOW_ALL设置为true的时候，可以不谢）
		5, CORS_ALLOW_METHODS = (
				'DELETE',
				'GET',   ##（获取数据）
				'OPTIONS',
				'PATCH',  ##补丁。更新数据。（django不支持,局部更新）
				'POST',   ##（创建数据）
				'PUT', ##变量全部改变（全部更新）
				)
		6, CORS_ALLOW_HEADERS = (
				...
				'origin',
				'user-agent',
				'x-csrftoken',  ##（x-表示第三方规定，非正规协议）
				'x-requested-with',
			)
		7, CORS_PREFLIGHT_MAX_AGE  默认 86400s   ##option请求缓存时间。
		8, CORS_EXPOSE_HEADERS  []   ##服务段自定义请求
		9, CORS_ALLOW_CREDENTIALS  布尔值， 默认False
        10，若是前后端完全分离，则禁用csrfmiddleware.
```
