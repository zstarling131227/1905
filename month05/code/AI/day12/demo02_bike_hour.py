...
# 加载hour数据
# 方法1
# data = np.loadtxt('../ml_data/bike_hour.csv', dtype='U20', usecols=(), unpack=True, delimiter=',')
# print(data)
# 方法2
header, data = [], []
...
mp.figure('BIKE hour Feature Importance', facecolor='lightgray')
mp.subplot(211)
mp.title('BIKE hour FI', fontsize=12)
mp.ylabel('Importance', fontsize=12)
mp.tick_params(labelsize=10)
mp.grid(linestyle=':')
xs = np.arange(len(random_fi))
sorted_index = random_fi.argsort()[::-1]  # 返回的是排序后的下标
mp.bar(xs, random_fi[sorted_index], color='red', label='BIKE hour FI')
