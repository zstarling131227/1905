"""
demo03_gscv.py GridSearchCV网格搜索
"""
import numpy as np
...
params = [
    {'kernel': ['linear'], "C": [1, 10, 100, 1000]},  # 4种排列组合
    {'kernel': ['poly'], "C": [1], 'degree': [2, 3]},  # 2种排列组合
    {'kernel': ['rbf'], "C": [1, 10, 100, 1000], 'gamma': [1, 0.1, 0.01, 0.001]}  # 16种排列组合
]
model = ms.GridSearchCV(model, params, cv=5)

model.fit(train_x, train_y)

# 获取GridSearchCV网格搜索的副产品
print(model.best_params_)  # 最优参数
print(model.best_score_)  # 最优分数
print(model.best_estimator_)  # 最优模型
# 输出每组超参数的测试集得分
