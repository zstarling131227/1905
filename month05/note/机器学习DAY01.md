...
股价预测(回归)、自然语言识别(分类)、语音识别(分类)、图像识别(分类)、人脸识别(分类)、推荐引擎
...
4)降维问题：在性能损失尽可能小的前提下，降低数据的复杂度。（跟图片结合较多）
...
为样本特征的每个值建立一个由一个1和若干个0组成的序列，用该序列对所有的特征值进行编码。（离散的值适合做独热编码，连续数据的不适合）
...
#绝对值拐点处不一定可微，而平方和没有这个顾虑。1/2只是为了求导之后约掉方便，对误差没有影响
...
*loss是关于w<sub>0</sub>和w<sub>1</sub>的函数*（三维图形）
